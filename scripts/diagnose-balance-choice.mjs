...
const shard=Number(process.env.SHARD),count=Number(process.env.COUNT??50);
for(let i=0;i<count;i++){
  const index=shard*count+i,city=cities[index%cities.length],seed=`平衡-${index}-${city}`;
    ...
    if(s.pending){
      ...
      try{s=choose(s,option);}catch(error){
        console.error('BALANCE_FAILURE '+JSON.stringify({
          shard,index,seed,city,age:s.age,eventId:s.pending?.id??s.pending?.eventId,title:s.pending?.title,
...
console.log(`shard ${shard} no failure`);
